...
public class CR16B {
    public static void main(String[] args) throws Exception{
        ...
        long ll = Integer.MIN_VALUE, rr = Integer.MAX_VALUE;
        long suml = calcSum(x, ll), sumr =calcSum(x, rr);
        while (ll < rr - 2) {
            //System.out.println(ll + " " + rr);
            long lmid = ll + (rr - ll) / 3;
            long rmid = rr - (rr - ll) / 3;
            long sumlmid = calcSum(x, lmid);
            long sumrmid = calcSum(x, rmid);
            if (sumlmid > sumrmid)
                ll = lmid;
            else
                rr = rmid;
...
public class B {
	...
	public void solve() {
		...
		long lo = -INF, hi = INF;
		long sumLo = calcSum(a, lo);
		long sumHi = calcSum(a, hi);

		while (hi - lo > 2) {
			long mid1 = lo + (hi - lo) / 3;
			long mid2 = hi - (hi - lo) / 3;
			long sumMid1 = calcSum(a, mid1);
			long sumMid2 = calcSum(a, mid2);
			if (sumMid1 > sumMid2)
				lo = mid1;
			else
				hi = mid2;
